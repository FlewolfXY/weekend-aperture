...
## Mobile viewport revision

The original 390×844 capture was entirely covered by the portrait-only rotation prompt, which made the artwork appear missing or half-screen. The portrait blocker has been removed. The revised 390×844 layout renders the complete experience inside one dynamic viewport: approximately three full day frames, an upright weekend pair, one weekday workstation, compact top HUD, reachable right-side controls, focus entry, and the full lunar/solar scrubber at the bottom. Mobile canvas geometry now uses wider frames and a shorter film band instead of compressing the six-frame desktop composition.
